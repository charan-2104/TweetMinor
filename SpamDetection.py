...
def naiveBayes():
    ...
    cvv = CountVectorizer(vocabulary=cv.get_feature_names_out(), stop_words="english", lowercase=False)
...
def accuracyComparison():
    height = [random_acc,nb_acc,svm_acc]
    bars = ('Random Forest Accuracy', 'Naive Bayes Accuracy','SVM Accuracy')
...
def graph():
    height = [total, fake_acc, spam_acc]  # The data points you want to visualize
    bars = ('Total Accounts', 'Fake Accounts', 'Spam Accounts')  # Labels for the bars
    y_pos = np.arange(len(bars))  # Position of each bar

    # Plotting the bar chart
    plt.bar(y_pos, height, color=['blue', 'orange', 'green'])
    
    # Adding the labels and title
    plt.xticks(y_pos, bars)
    plt.ylabel('Count')
    plt.title('Comparison of Account Types')
...
detectButton.place(x=520,y=250)
detectButton.config(font=font1) 
